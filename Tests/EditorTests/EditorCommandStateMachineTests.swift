...
class EditorCommandStateMachineTests: XCTestCase {
    ...
    func testStandaloneCommands() {
        ...
        XCTAssertTrue(editor.log.last?.contains("deleteToEndOfLine") ?? false)

        stateMachine.handleToken(.deleteChar, editor: editor)
        XCTAssertTrue(editor.log.last?.contains("deleteCurrentCharacter") ?? false)
        ...
        // a
        stateMachine.handleToken(.switchToInsertModeAfterCursor, editor: editor)
        XCTAssertEqual(editor.log.last, "switchToInsertMode()")
...
extension EditorCommandStateMachineTests {
    ...
    func testYankLinewise() {
        ...
        XCTAssertEqual(
            editor.cursorPosition, 0, "Cursor should move to beginning of the line after yy")
        ...
        XCTAssertEqual(
            editor.cursorPosition, 4, "Cursor should move to beginning of the first yanked line")
    }
...
extension EditorCommandStateMachineTests {
    ...
    func testChangeToEndOfLine_Counted() {
        ...
        XCTAssertEqual(editor.cursorPosition, 4)
        XCTAssertTrue(
            editor.log.contains("switchToInsertMode()"), "Should switch to insert mode after C")
    }
...
extension EditorCommandStateMachineTests {
    func testRepeatFindForward() {
        ...
        XCTAssertEqual(editor.cursorPosition, 6)  // Stays put on first char
